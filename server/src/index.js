...
const cryptoHelper = require('./brokers/cryptoHelper');
...
// Static frontend serving relocated below /api routes
...
app.listen(PORT, () => {
  ...
  // Initialize tables on startup and schedule local cron
  initTables().then(() => {
    startLocalCron();
  }).catch(err => {
    console.error("Failed to run startup migrations / cron setup:", err.message);
  });
});
